...
def deeplab_v3_plus_generator(num_classes,
    ...
    @nvtx_tf.ops.trace(message='DEEPLAB_GEN', domain_name='Forward', grad_domain_name='Gradient')
                        ...
                        if decoder == 'bilinear':
                            ...
                            if decoder == 'deconv1x':
                                ...
                                skip = tf.layers.conv2d(inputs, 64, [3, 3],
				                        ...
				                        bias_initializer=tf.initializers.zeros(),
                                                        kernel_regularizer=tf.contrib.layers.l2_regularizer(scale=_WEIGHT_DECAY))
#                                skip, nvtx_context = nvtx_tf.ops.start(skip, message='DCONNNNNNN',
#                                        domain_name='Forward', grad_domain_name='Gradient')
                                ...
                                net = tf.layers.conv2d(net, 256, [3, 3],
				                       ...
				                       bias_initializer=tf.initializers.zeros(),
                                                       kernel_regularizer=tf.contrib.layers.l2_regularizer(scale=_WEIGHT_DECAY))
#                                net = nvtx_tf.ops.end(net, nvtx_context)
                                net = tf.layers.conv2d(net, 256, [3, 3],
                                                       padding='same',
                                                       data_format='channels_last',
				                       kernel_initializer=tfk.initializers.he_uniform(),
